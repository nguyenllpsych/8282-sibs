...
```{r demo, results = "asis", warning = FALSE, message = FALSE}
#descriptives
mpq %>% 
...
```{r adtop-data, warning = F, message = F}
...
There are `r nrow(yo[which(yo$adopt == 1),])` adopted pairs and `r nrow(yo[which(yo$adopt == 0),])` non-adopted sibling pairs.

### Adoption status - Aggression AG

```{r adopt-AG, warning=FALSE, message=FALSE, cache = T}
...
```{r adopt-CON, warning=FALSE, message=FALSE, cache = T}
...
```{r adopt-HA, warning=FALSE, message=FALSE, cache = T}
...
```{r sex-data, warning = F, message = F}
...
There are `r nrow(yo[which(yo$samesex == 1),])` same-sex and `r nrow(yo[which(yo$samesex == 0),])` opposite-sex sibling pairs.

### Sex similarity - Aggression AG

```{r sex-AG, warning=FALSE, message=FALSE, cache = T}
...
```{r sex-CON, warning=FALSE, message=FALSE, cache = T}
...
```{r sex-HA, warning=FALSE, message=FALSE, cache = T}
...
```{r lgm-AG-full, warning = F, cache = T}
lgmAG <- '
#younger sibling
AG0y =~   AG_IN_y_p1  + p2 * AG_IN_y_p2   + p3 * AG_IN_y_p3
AG3y =~   AG_FU1_y_p1 + p2 * AG_FU1_y_p2  + p3 * AG_FU1_y_p3
...
```{r lgm-CON-full, warning = F, cache = T}
lgmCON <- '
#younger sibling
CON0y =~   CON_IN_y_p1  + p2 * CON_IN_y_p2   + p3 * CON_IN_y_p3
CON3y =~   CON_FU1_y_p1 + p2 * CON_FU1_y_p2  + p3 * CON_FU1_y_p3
...
```{r lgm-HA-full, warning = F, cache = T}
lgmHA <- '
#younger sibling
HA0y =~   HA_IN_y_p1  + p2 * HA_IN_y_p2   + p3 * HA_IN_y_p3
HA3y =~   HA_FU1_y_p1 + p2 * HA_FU1_y_p2  + p3 * HA_FU1_y_p3
...
```{r adopt-AG-full, warning = F, message = F, cache = T}
...
```{r adopt-CON-full, warning = F, message = F, cache = T}
# extract latent intercepts and slopes
...
```{r adopt-HA-full, warning = F, message = F, cache = T}
# extract latent intercepts and slopes
...
```{r sex-AG-full, warning=F, message=F, cache=T}
# extract latent intercepts and slopes
...
```{r sex-CON-full, warning=F, message=F, cache=T}
# extract latent intercepts and slopes
...
```{r sex-HA-full, warning=F, message=F, cache=T}
# extract latent intercepts and slopes
...
```{r age-data, warning = F, message = F}
# descriptives
...
Sibling relationship was measured by the Sibling Relationship Questionnaire (SRQ; [Furman, & Buhrmester, 1985](https://www.jstor.org/stable/1129733)). The scale has 48 items. We excluded items 2, 18, 34 (subscale maternal partiality), and items 7, 23, 39 (subscale paternal partiality) during data collection. This was measured at the first two waves: IN and FU1. There are a total of 14 subscales, each with 3 items, and they can be organized into 3 (overlapping) factors. The original paper included a 4th factor, Rivalry, which consists of Competition and Parental partiality (which we did not measure).
...
We will use relationship quality data at intake for all participants. However, there are `r length(rel_FU1[which(!rel_FU1$ID %in% rel_IN$ID), "ID"])` participants who did not have relationship data at IN but had FU1 data that will be used instead.
...
```{r rela-AG-young, warning = F, message = F}
#### younger siblings
...
```{r rela-AG-old, warning = F, message = F}
#### older siblings
...
```{r rela-CON-young, warning = F, message = F}
#### younger siblings
...
```{r rela-CON-old, warning = F, message = F}
#### older siblings
...
Life events data were collected from a Life Event Interview at intake, follow-up 1, and follow-up 2. These events were categorized into 3 groups following previous research by[Billig et al., 1996](https://link.springer.com/article/10.1007/BF02361227):

```{r lei-data, warning = F, cache = T}
