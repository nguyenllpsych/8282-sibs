...
as.data.frame(unclass(parameterEstimates(sibAG, standardized = T))) %>% 
...
as.data.frame(unclass(parameterEstimates(sibCON, standardized = T))) %>% 
  filter(op == "~1" & lhs %in% c("iy", "io", "sy", "so")) %>% 
  select("Parameters" = lhs, B = est, SE = se, Z = z, "p-value" = pvalue, "CI lower" = ci.lower, "CI upper" = ci.upper) %>% 
  knitr::kable(caption = "Intercept and Slope", digits = 6) %>% 
  kable_styling() %>% 
...
as.data.frame(unclass(parameterEstimates(sibHA, standardized = T))) %>% 
  filter(op == "~1" & lhs %in% c("iy", "io", "sy", "so")) %>% 
  select("Parameters" = lhs, B = est, SE = se, Z = z, "p-value" = pvalue, "CI lower" = ci.lower, "CI upper" = ci.upper) %>% 
  knitr::kable(caption = "Intercept and Slope", digits = 6) %>% 
  kable_styling() %>% 
...
# intercept and slope 
as.data.frame(unclass(parameterEstimates(adoptAG, standardized = T))) %>% 
  filter(op == "~1" & lhs %in% c("iy", "io", "sy", "so")) %>% 
...
# intercept and slope 
as.data.frame(unclass(parameterEstimates(adoptCON, standardized = T))) %>% 
  filter(op == "~1" & lhs %in% c("iy", "io", "sy", "so")) %>% 
  select("Adopted" = group, "Parameters" = lhs, B = est, SE = se, Z = z, "p-value" = pvalue, "CI lower" = ci.lower, "CI upper" = ci.upper) %>% 
  mutate(Adopted = ifelse(Adopted == 1, "adopt", "bio")) %>% 
  arrange(Parameters) %>% 
...
# intercept and slope 
as.data.frame(unclass(parameterEstimates(adoptHA, standardized = T))) %>% 
  filter(op == "~1" & lhs %in% c("iy", "io", "sy", "so")) %>% 
  select("Adopted" = group, "Parameters" = lhs, B = est, SE = se, Z = z, "p-value" = pvalue, "CI lower" = ci.lower, "CI upper" = ci.upper) %>% 
  mutate(Adopted = ifelse(Adopted == 1, "adopt", "bio")) %>% 
  arrange(Parameters) %>% 
...
# intercept and slope 
as.data.frame(unclass(parameterEstimates(sexAG, standardized = T))) %>% 
  filter(op == "~1" & lhs %in% c("iy", "io", "sy", "so")) %>% 
  select("SameSex" = group, "Parameters" = lhs, B = est, SE = se, Z = z, "p-value" = pvalue, "CI lower" = ci.lower, "CI upper" = ci.upper) %>% 
...
as.data.frame(unclass(parameterEstimates(sexAG, standardized = T))) %>% 
  filter(op == "~~" & lhs %in% c("sy", "iy") & rhs %in% c("so", "io") & est > 0) %>% 
  arrange(lhs) %>% 
  select("SameSex" = group, "Younger sibling" = lhs, "Older sibling" = rhs, B = est, SE = se, Z = z, "p-value" = pvalue, "CI lower" = ci.lower, "CI upper" = ci.upper) %>% 
  mutate(SameSex = ifelse(SameSex == 1, "same-sex", "opposite-sex")) %>% 
  knitr::kable(caption = "Correlated intercepts and slopes", digits = 6) %>% 
  kable_styling() %>% 
  footnote(general = "i = intercept, s = slope, o = older sibling, y = younger sibling")
...
# intercept and slope 
as.data.frame(unclass(parameterEstimates(sexCON, standardized = T))) %>% 
  filter(op == "~1" & lhs %in% c("iy", "io", "sy", "so")) %>% 
  select("SameSex" = group, "Parameters" = lhs, B = est, SE = se, Z = z, "p-value" = pvalue, "CI lower" = ci.lower, "CI upper" = ci.upper) %>% 
  mutate(SameSex = ifelse(SameSex == 1, "same-sex", "opposite-sex")) %>% 
  arrange(Parameters) %>% 
...
# intercept and slope 
as.data.frame(unclass(parameterEstimates(sexHA, standardized = T))) %>% 
  filter(op == "~1" & lhs %in% c("iy", "io", "sy", "so")) %>% 
  select("SameSex" = group, "Parameters" = lhs, B = est, SE = se, Z = z, "p-value" = pvalue, "CI lower" = ci.lower, "CI upper" = ci.upper) %>% 
  mutate(SameSex = ifelse(SameSex == 1, "same-sex", "opposite-sex")) %>% 
  arrange(Parameters) %>% 
